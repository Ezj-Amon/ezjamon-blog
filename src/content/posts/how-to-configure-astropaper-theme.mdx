...
| `dynamicOgImage`   | Generate a dynamic OG image per post when no `ogImage` is specified in frontmatter. Defaults to `true`. See the [trade-off](https://ezjamon.com/posts/dynamic-og-image-generation-in-astropaper-blog-posts/#trade-off) for details. |
...
- [Customizing AstroPaper theme color schemes](https://ezjamon.com/posts/customizing-astropaper-theme-color-schemes/) — change or add color schemes via `src/styles/theme.css`.
- [Adding new posts](https://ezjamon.com/posts/adding-new-posts-in-astropaper-theme/) — frontmatter reference and file conventions.
